...
public:
	...
	friend bool operator==(const Aktivnost& a1, const Aktivnost& a2) {
		if (a1._razred == a2._razred && a1._opis == a2._opis && a1._ocjena == a2._ocjena)
			return true;
		return false;
	}
};
...
public:
	...
	void PredstaviSe() { cout << *this << endl; }
};
class Skola {
	char* _naziv;
	...
	void operator()(string imePrezime, string brojTelefona) {
		for (size_t i = 0; i < _ucenici.size(); i++)
		{
			if (strcmp(imePrezime.c_str(), _ucenici[i].GetImePrezime()) == 0 && brojTelefona == _ucenici[i].GetTelefon())
				throw exception("Pokusaj dupliranja ucenika!\n");
			...
			if (strcmp(imePrezime.c_str(), _ucenici[i].GetImePrezime()) == 0)
			{
				for (size_t j = 0; j < _ucenici[i].GetAktivnosti().GetTrenutno(); j++)
				{
					if (_ucenici[i].GetAktivnosti().GetElement2(j).GetRazred() == aktivnost.GetRazred() && _ucenici[i].GetAktivnosti().GetElement1(j) == predmet)
						return false;
					if (_ucenici[i].GetAktivnosti().GetElement2(j).GetRazred() == aktivnost.GetRazred() && _ucenici[i].GetAktivnosti().GetElement2(j).GetOcjenu()) {
						brojac++;
						suma += _ucenici[i].GetAktivnosti().GetElement2(j).GetOcjenu();
					}
					if (brojac >= 4)
						return false;
				}
				...
				brojac++;
				if (brojac <= 3 && _ucenici[i].GetTelefon() != not_set) {
					suma += aktivnost.GetOcjenu();
					suma /= brojac;
					thread t([&]() {
						cuvar.lock();
						cout << "Cestitamo na uspjehu koji ste ostvarili u " << aktivnost.GetRazred() << " razredu. Vas uspjeh iznosi: " << suma << endl;
						cuvar.unlock();
						});
					t.join();
				}
